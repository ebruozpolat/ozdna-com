...
| PDQ-256 | `pdq-wasm` **0.3.9** + vendored `app/vendor/pdq-wasm/` | Pure distance/encoding/scoring in `@ozdna/dna-core` (`pdq.ts`, `match.ts`, `bandProbes`); Node `@ozdna/dna-core/pdq-node`; browser `@ozdna/dna-core/pdq-browser`; Workers `@ozdna/dna-core/pdq-worker` | Serve vendored `pdq.wasm` (never CDN). Vendor dir has `"type":"commonjs"` so Emscripten glue `require`s cleanly. |
