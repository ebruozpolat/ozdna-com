...
|---|---|---|---|---|
| 0.1 | Email conformance@c2pa.org: ask (a) Level 1 conformance program cost + timeline for a small Generator product, and (b) whether a conformant service can obtain **per-tenant end-entity claim-signing certs**, and at what per-cert price (trust-list CAs issue end-entity certs only — see `plan/01-ARCHITECTURE.md` §9) | TODO | $0 | The single unknown number in the plan. Answer may take weeks — send first. |
| 0.2 | Join Content Authenticity Initiative (contentauthenticity.org/membership) | TODO | $0 | Free. Credibility line for whitepaper + grants. |
| 0.3 | ozdna.com landing page + waitlist, two segments: (a) AI companies needing EU Art. 50 marking, (b) sellers/creators | IN PROGRESS | $0 | THE demand validation. Built Jul 8 (`/products/origin/` + segmented Netlify form, branch `claude/ozdna-proof-of-origin-8woql9`); goes live on merge to `main` + Netlify repo-connect (see `docs/DOMAIN.md`). |
...
| 0.9 | Partner conversation + domain: OriginDNA owns ozDNA brand long-term; platform site stays until OriginDNA landing ships; humanizer never under ozDNA | IN PROGRESS | ~$10 domain | Domain released from alignxmedia + attached to ozdna-614 (Jul 7). Repurposed content built Jul 8 (umbrella + OriginDNA waitlist, on branch); log DONE once merged to `main` + Netlify repo-connect done. |
