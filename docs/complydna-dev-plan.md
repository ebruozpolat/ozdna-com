...
**Kullanım:** Önce Cursor Rules'u projeye kaydet, sonra prompt'ları sırayla ver.
...
## 0) Cursor Rules (`.cursor/rules/complydna.mdc` olarak kaydet)

```
Sen ComplyDNA'nın geliştiricisisin. ComplyDNA, Türk mevzuatına (MASAK, 5549,
6415, KVKK, ROM Yönetmeliği) özel, kaynak künyeli yanıt üreten bir uyum LLM
ürünüdür.

Değişmez kurallar:
- Dil: Backend Python 3.11 + FastAPI, tip anotasyonları zorunlu, pydantic v2.
- Vektör DB: Qdrant (docker-compose ile lokal). Embedding: çok dilli model (örn. bge-m3).
- Her mevzuat parçası (chunk) şu metadata'yı taşır: kaynak_kodu (örn. LAW-5549),
  madde_no, fıkra, yürürlük_durumu, yürürlük_tarihi, sürüm.
- Model yanıtındaki HER cümle [KAYNAK_KODU / Madde X] künyesi taşımalı;
  künyesiz iddia üretmek hata sayılır.
- Test: pytest, her modülün testi modülle aynı PR'da yazılır.
- Müşteri verisi asla eğitim/log setine yazılmaz; PII loglanmaz.
- Sır (API key vb.) koda gömülmez, .env + pydantic-settings kullanılır.
```
