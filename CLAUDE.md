...
> **Second product line — same ozDNA umbrella, do not conflate:** `docs/oversight/` holds the planning corpus for a distinct **"AI oversight infrastructure"** product (dated Jul 25, 2026) that reuses the names *ozDNA / originDNA / complyDNA* for entirely different things (AI Act Art. 9/12/13/14 runtime — classification, Council oversight, decision Ledger, Attestation). Per founder decision both live under the **single ozDNA / ozdna.com umbrella**, split by path: content-provenance stays at `/`, `/products/*`, `/verify`; the oversight site is served at **`ozdna.com/oversight/`** (source in repo-root `oversight/`). **AlignX Partners is the founder's separate personal UK consulting business (alignxpartners.com), currently in formation (UK company not yet formed) — NOT the ozDNA umbrella and carries no ozDNA content.** This file (OriginDNA / content-provenance) stays authoritative for its own line. See `docs/oversight/README.md` for the split and the deferred name-collision decision.
